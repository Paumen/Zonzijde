...
Purpose
A viewer can watch the newspaper being made and come away understanding how the pipeline turns raw input into a finished edition.
The visualization is compelling enough to demo and share as a way of explaining the system, to a non-technical person.
The experience feels alive — like watching a newsroom work — rather than reading a dashboard or log.

Fidelity to the real run
Every headline, date, score, werktitel, and artikelkop shown is taken from a real pipeline run, never invented, or incomplete.
The counts and visuals at every stage match the real run's actual numbers.
...
The full volume of incoming items is conveyed, not a tiny sample, so the scale of the funnel is felt.
All real items are shown at each stage, rather than a token few.
...
The four geographic rings (lokaal, regionaal, nationaal, internationaal) are visible and their balance across the edition is apparent.
The paper itself is seen taking shape. The edition is perceived as an object gradually emerging from the pipeline rather than appearing only at the end. Werktitels becoming text, gaining final titles, receiving an illustration, and ending as a finished page.

Why things happen 
Whenever items are filtered out, the reason is visible and intuitive, not just their disappearance.
Items removed for being outside the date window are clearly seen leaving because they are too old.
Duplicates and negatively-filtered items are each shown leaving for their own distinct, labelled reason.
When an LLM performs a step, it is unmistakable that a model is doing the work.
An LLM's per-item progress is visible as it works through a step, including its batching where that is how it runs.
...
The intake ramps up gradually, like scraping and gathering, rather than beginning at full volume instantly.
The handoff from one stage to the next is smooth and continuous, with no jarring cut.
The viewer's attention is naturally directed toward the active work without losing awareness of the whole pipeline.
