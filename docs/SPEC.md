...
| PIPE-4 | Select | Items scoring +1/+2 go to an LLM with the brief; output is 5 *topics* per scope (unranked), one row per source article (a topic covered by multiple sources gets multiple rows), columns: bron, scope, titel, samenvatting, link. |
