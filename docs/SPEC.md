...
| PIPE-4 | Select | Positively scored items go to an LLM with the brief; output is the onderwerpen per scope, unranked, one row per source article, columns: medium, scope, bron_titel, bron_samenvatting, bron_link. Counts per scope: `config/edition.yaml` → `edition.onderwerpen`. Grouping: `select.md`. |
...
Configured values live in `config/edition.yaml` and are templated into `select.md` and
`outline.md`.
