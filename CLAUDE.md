...
- **No summary fallback** (PIPE-5): an RSS blurb is never writing material. Blocked
  stories are re-sourced or dropped and logged. Note: the prototype
  `tools/fetch-articles.py` still falls back to the RSS `samenvatting`; that behaviour
  is explicitly dropped when it becomes stage S5 (ARCHITECTURE §1) — don't carry it over.
